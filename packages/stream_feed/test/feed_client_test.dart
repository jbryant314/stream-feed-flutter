...
void main() {
  group('Feed Client', () {
    ...
    test('addActivity', () async {
      ...
      when(() => api.addActivity(token, feedId, activity)).thenAnswer((_) async => activity);
    ...
    test('addActivities', () async {
      ...
      when(() => api.addActivities(token, feedId, activities)).thenAnswer((_) async => activities);
    ...
    test('unfollow', () async {
      ...
      when(() => api.unfollow(token, feedId, feedId, keepHistory: true)).thenAnswer((_) async => dummyResponse);
      await client.unfollow(flatFeed, keepHistory: true);
      // ignore: invalid_use_of_protected_member
      verify(() => api.unfollow(token, feedId, feedId, keepHistory: true)).called(1);
    });
    ...
    test('followStats', () async {
      ...
      when(() => api.followStats(token, options.toJson())).thenAnswer((_) async => FollowStats.fromJson(jsonFixture('follow_stats.json')));
      await client.followStats(followerSlugs: ['user', 'news'], followingSlugs: ['timeline']);
    ...
    test('getFollowed', () async {
      ...
      when(() => api.following(token, feed, limit, offset, feedIds)).thenAnswer((_) async => follows);
      await client.following(limit: limit, offset: offset, filter: feedIds);
      verify(() => api.following(token, feed, limit, offset, feedIds)).called(1);
    });
    ...
    test('getFollowers', () async {
      ...
      when(() => api.followers(token, feed, limit, offset, feedIds)).thenAnswer((_) async => follows);
      await client.followers(limit: limit, offset: offset, feedIds: feedIds);
      verify(() => api.followers(token, feed, limit, offset, feedIds)).called(1);
    });
    ...
    test('updateActivityById', () async {
      ...
      const actitivity = Activity(actor: 'actor', verb: 'verb', object: 'object');
      when(() => api.updateActivityById(token, update)).thenAnswer((_) async => actitivity);
      await client.updateActivityById(id: update.id!, set: update.set, unset: update.unset);
    ...
    test('updateActivitiesById', () async {
      ...
      const actitivities = [Activity(actor: 'actor', verb: 'verb', object: 'object')];
      when(() => api.updateActivitiesById(token, updates)).thenAnswer((_) async => actitivities);
    ...
    test('updateActivitiesByForeignId', () async {
      ...
      final updates = [ActivityUpdate.withId(id: id, set: set, unset: unset)];
      const actitivities = [Activity(actor: 'actor', verb: 'verb', object: 'object')];
      when(() => api.updateActivitiesByForeignId(token, updates)).thenAnswer((_) async => actitivities);
    ...
    test('updateActivityByForeignId', () async {
      ...
      const actitivity = Activity(actor: 'actor', verb: 'verb', object: 'object');
      when(() => api.updateActivityByForeignId(token, update)).thenAnswer((_) async => actitivity);
    ...
    test('updateActivityToTargets', () async {
      ...
      final remove = <FeedId>[];
      final feed = FeedId('slug', 'userId');
      when(() => api.updateActivityToTargets(token, feed, update, add: add, remove: remove)).thenAnswer((_) async => dummyResponse);
      await client.updateActivityToTargets(update, add, remove);
      verify(() => api.updateActivityToTargets(token, feed, update, add: add, remove: remove)).called(1);
    });

    test('removeActivityByForeignId', () async {
      const foreignId = 'foreignId';
      when(() => api.removeActivityByForeignId(token, feedId, foreignId)).thenAnswer((_) async => dummyResponse);
      await client.removeActivityByForeignId(foreignId);
      verify(() => api.removeActivityByForeignId(token, feedId, foreignId)).called(1);
    });

    test('removeActivityById', () async {
      const id = 'foreignId';
      when(() => api.removeActivityById(token, feedId, id)).thenAnswer((_) async => dummyResponse);
    ...
    test('replaceActivityToTargets', () async {
      ...
      final feed = FeedId('slug', 'userId');
      when(() => api.updateActivityToTargets(token, feed, update, replace: newTargets)).thenAnswer((_) async => dummyResponse);
      await client.replaceActivityToTargets(update, newTargets);
      verify(() => api.updateActivityToTargets(token, feed, update, replace: newTargets)).called(1);
    });
  });
}
